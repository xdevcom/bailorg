...
const makeSocket = (config) => {
    ...
    const onUnexpectedError = (err, msg) => {
        ...
        // gently back off when encountering rate limits (429) with exponential backoff
        if (message.includes('429') || message.includes('rate limit')) {
            _backoffCount++;
            const base = config.backoffDelayMs || 3000;
            const wait = Math.min(60000, base * Math.pow(1.5, Math.min(_backoffCount - 1, 8)));
            logger.info({ wait, attempt: _backoffCount }, 'backing off due to rate limit');
            setTimeout(() => {
                // intentionally empty; wait to delay further sends
            }, wait);
        } else {
            // reset backoff counter on successful non-rate-limit errors
            _backoffCount = Math.max(0, _backoffCount - 1);
        }
    };
    ...
    const uploadPreKeysToServerIfRequired = async (force) => {
