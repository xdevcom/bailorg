...
class WebSocketClient extends abstract_socket_client_1.AbstractSocketClient {
    ...
    async connect() {
        ...
        this.socket.on('ping', () => {
            var _a;
            try {
                (_a = this.socket) === null || _a === void 0 ? void 0 : _a.pong();
            }
            catch (_e) {
                // ignore pong errors
            }
    ...
    async close() {
        ...
        this.socket = null;
        // graceful close: give it 3 seconds then force destroy
        try {
            sock.close();
            await new Promise((resolve) => {
                const timer = setTimeout(() => {
                    try { sock.terminate(); } catch (_e) { /* ignore */ }
                    resolve();
                }, 3000);
            ...
            try { sock.terminate(); } catch (_e2) { /* ignore */ }
        }
    }
    send(str, cb) {
        var _a;
