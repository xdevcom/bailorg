...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            // try each host up to 2 times before moving on
            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    ...
                    const isBodyTooLarge = (error === null || error === void 0 ? void 0 : error.output) && (error === null || error === void 0 ? void 0 : error.output.statusCode) === 413;
                    if (isBodyTooLarge || attempt >= 1) {
                        const isLast = hostname === ((_b = hosts[uploadInfo.hosts.length - 1]) === null || _b === void 0 ? void 0 : _b.hostname);
                        logger.warn({ trace: error.stack, uploadResult: result }, `Error in uploading to ${hostname} ${isLast ? '' : ', retrying...'}`);
                        break;
                    }
                    // wait briefly before retry on same host
                    logger.debug(`Retrying upload to "${hostname}" after failure (attempt ${attempt + 1})`);
